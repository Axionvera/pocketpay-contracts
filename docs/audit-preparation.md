...
- [x] **Storage Key Separation**:
  - [x] Instance storage is used for metadata: `Admin`, `Initialized`, `Token` in [lib.rs](file:///c:/Users/User/OneDrive/Desktop/GrantFox/pocketpay-contracts/contracts/savings_vault/src/lib.rs).
  - [x] Persistent storage is used for user state: `Balance(Address)`, `Lock(Address, u64)`, `NextLockId(Address)`.
- [x] **Unbounded Vector Growth (DoS Vector)**:
  - [x] **MITIGATED**: Locks are stored under separate individual storage keys (`DataKey::Lock(Address, u64)`), ensuring $O(1)$ lock access and avoiding large single vector serialization DoS risks.
  - [x] **Storage Key Design**: Each lock has a stable, unique ID generated by monotonically increasing `NextLockId(user)`, preventing key collisions across users.
