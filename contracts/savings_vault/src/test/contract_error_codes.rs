...
fn error_code_1001_amount_not_positive_on_deposit() {
    ...
    assert!(
        msg.contains(&ContractError::AmountNotPositive as u32.to_string())
...
fn error_code_1002_unlock_time_not_in_future_lock_funds() {
    ...
    assert!(
        msg.contains(&ContractError::UnlockTimeNotInFuture as u32.to_string())
...
fn error_code_1003_lock_duration_exceeds_maximum() {
    ...
    set_ledger_timestamp(&env, 1_000);
    client.set_max_lock_duration(&token_admin, &1_000);
    let user = Address::generate(&env);
    let msg = catch_panic_message(|| client.lock_funds(&user, &10, &10_000));
    assert!(
        msg.contains(&ContractError::LockDurationExceedsMaximum as u32.to_string())
...
fn error_code_1004_lock_duration_below_minimum() {
    ...
    let (_contract_id, client, _token_client, token_admin, _) = vault_with_sac(&env);
    set_ledger_timestamp(&env, 1_000);
    client.set_min_lock_duration(&token_admin, &5_000);
    let user = Address::generate(&env);
    let msg = catch_panic_message(|| client.lock_funds(&user, &10, &2_000));
    assert!(
        msg.contains(&ContractError::LockDurationBelowMinimum as u32.to_string())
...
fn error_code_1005_amount_below_minimum_deposit() {
    let env = test_env();
    let (_contract_id, client, _token_client, token_admin, _) = vault_with_sac(&env);
    client.set_min_deposit_amount(&token_admin, &1_000);
    let user = Address::generate(&env);
    let msg = catch_panic_message(|| client.deposit(&user, &50));
    assert!(
        msg.contains(&ContractError::AmountBelowMinimumDeposit as u32.to_string())
...
fn error_code_1006_pause_duration_must_be_positive() {
    let env = test_env();
    let (_contract_id, client, _token_client, token_admin, _) = vault_with_sac(&env);
    let msg = catch_panic_message(|| client.pause(&token_admin, &0));
    assert!(
        msg.contains(&ContractError::PauseDurationMustBePositive as u32.to_string())
...
fn error_code_1007_min_deposit_amount_negative() {
    let env = test_env();
    let (_contract_id, client, _token_client, token_admin, _) = vault_with_sac(&env);
    let msg = catch_panic_message(|| client.set_min_deposit_amount(&token_admin, &-1));
    assert!(
        msg.contains(&ContractError::MinDepositAmountNegative as u32.to_string())
...
fn error_code_2001_not_authorized_admin_wrong_caller() {
    ...
    assert!(
        msg.contains(&ContractError::NotAuthorizedAdmin as u32.to_string())
...
fn error_code_3001_already_initialized() {
    ...
    let msg = catch_panic_message(|| client.initialize(&token_admin, &other_token));
    assert!(
        msg.contains(&ContractError::AlreadyInitialized as u32.to_string())
...
fn error_code_3002_not_initialized_before_deposit() {
    ...
    assert!(
        msg.contains(&ContractError::NotInitialized as u32.to_string())
...
fn error_code_3003_contract_paused_blocks_deposit() {
    let env = test_env();
    let (_contract_id, client, _token_client, token_admin, _) = vault_with_sac(&env);
    client.pause(&token_admin, &60);
    let user = Address::generate(&env);
    let msg = catch_panic_message(|| client.deposit(&user, &100));
    assert!(
        msg.contains(&ContractError::ContractPaused as u32.to_string())
...
fn error_code_4001_insufficient_balance_withdraw() {
    ...
    assert!(
        msg.contains(&ContractError::InsufficientBalance as u32.to_string())
...
fn error_code_4002_insufficient_balance_to_lock() {
    ...
    assert!(
        msg.contains(&ContractError::InsufficientBalanceToLock as u32.to_string())
...
fn error_code_5001_lock_not_found_on_withdraw() {
    ...
    assert!(
        msg.contains(&ContractError::LockNotFound as u32.to_string())
...
fn error_code_5002_lock_already_withdrawn() {
    let env = test_env();
    let (contract_id, client, _token_client, _token_admin, asset_admin) = vault_with_sac(&env);
    let user = Address::generate(&env);
    set_ledger_timestamp(&env, 1_000);
    asset_admin.mint(&user, &10_000);
    env.mock_all_auths();
    ...
    assert!(
        msg.contains(&ContractError::LockAlreadyWithdrawn as u32.to_string())
...
fn error_code_5003_lock_not_matured() {
    ...
    let msg = catch_panic_message(|| client.withdraw_lock(&user, &id));
    assert!(
        msg.contains(&ContractError::LockNotMatured as u32.to_string())
...
fn error_code_5004_extend_lock_time_not_increased() {
    ...
    assert!(
        msg.contains(&ContractError::ExtendLockTimeNotIncreased as u32.to_string())
...
fn error_code_8001_cannot_transfer_admin_to_self() {
    let env = test_env();
    let (_contract_id, client, _token_client, token_admin, _) = vault_with_sac(&env);
    let same = token_admin.clone();
    let msg = catch_panic_message(|| client.transfer_admin(&token_admin, &same));
    assert!(
        msg.contains(&ContractError::CannotTransferAdminToSelf as u32.to_string())
...
fn error_code_8002_cannot_transfer_admin_to_contract_address() {
    let env = test_env();
    let (contract_id, client, _token_client, token_admin, _) = vault_with_sac(&env);
    let msg = catch_panic_message(|| client.transfer_admin(&token_admin, &contract_id));
    assert!(
        msg.contains(&ContractError::CannotTransferAdminToContractAddress as u32.to_string())
